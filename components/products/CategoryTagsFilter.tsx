...
export default function CategoryTagsFilter({ 
  categoryId,
  categorySlug,
  selectedTag 
}: CategoryTagsFilterProps) {
  const [tagsData, setTagsData] = useState<CategoryTagsData | null>(null)
  ...
  useEffect(() => {
    console.log('🏷️ CategoryTagsFilter - selectedTag:', selectedTag)
  }, [selectedTag])
  ...
  useEffect(() => {
    const fetchTags = async () => {
      ...
      const { data, error: fetchError } = await safeFetch<CategoryTagsData>(
        `/api/category-tags/${categoryId}`
      )
      
      if (fetchError || !data) {
        console.error('Error fetching category tags:', fetchError)
    ...
    return (
      <div className="p-4 mb-6 bg-white rounded-lg shadow-sm">
        <div className="flex items-center gap-2 text-gray-500">
          <div className="w-5 h-5 border-2 border-gray-300 rounded-full border-t-primary animate-spin" />
  ...
  // Build tag URL: /products/category/{categorySlug}/tag/{tagSlug}
  const getTagUrl = (tagSlug: string) => {
    return `/products/category/${categorySlug}/tag/${tagSlug}`
  }

  // Build category URL (clear filter)
            ...
            >
              ...
              <div className="p-6 text-white bg-gradient-to-r from-purple-600 to-blue-600">
                ...
                <p className="text-sm text-white/90">
                  {tagsData.total_tags} وسم متاح في {tagsData.category_name}
                </p>
              </div>
