...
export async function GET(
  ...
  try {
    ...
    console.log(`🏷️ Store Tag Products API called for store ${vendorId}, tag ${tagSlug}`)
    console.log(`✅ Returning ${data.products?.length || 0} products`)
    
    return NextResponse.json(data, { status: 200 })
  } catch (error) {
    console.error('❌ Error fetching store tag products:', error)
    
    return NextResponse.json(
      { 
        error: 'Failed to fetch store tag products',
